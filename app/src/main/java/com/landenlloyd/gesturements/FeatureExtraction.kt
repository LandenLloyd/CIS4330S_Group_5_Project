...
/**
 * The final features that are used to generate audio are a moving-average of accelerometer frame
 * features to make the sounds generated by the synthesizer more "consistent".
 * The gyroscope features just use the most recent.
 */
class GesturementsFeatures (movingAverageSize: Int = 12) {
    val accelValues = DoubleArray(movingAverageSize)
    var accelMean: Double = 0.0
    var gyroDeltaTime: Double = 0.0
    var gyroSum: Double = 0.0

    fun addFrames(accelFeatures: FrameFeatureExtractor, gyroFeatures: FrameFeatureExtractor) {
        // Collect Gyroscope features
        gyroDeltaTime = gyroFeatures.t.max - gyroFeatures.t.min
        gyroSum = gyroFeatures.z.sum

        // Shift all elements up one
        for (i in 0 until accelValues.size - 1) {
            accelValues[i] = accelValues[i+1]
        }

        // Add the new acceleration mean
        accelValues[accelValues.size - 1] = accelFeatures.magnitude.mean

        // Update the accel mean
        accelMean = accelValues.sum() / accelValues.size
    }
}
